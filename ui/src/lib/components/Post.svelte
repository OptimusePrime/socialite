<script lang="ts">
    ...
    import {
        countPostLikes, createFavourite,
        createLike, deleteFavourite, deleteLike,
        deletePost,
        DeletePostErrors,
        getSignedInUserId, isPostFavourited,
        isPostLiked
    ...
    onMount(async () => {
        ...
        isFavourite = await isPostFavourited(await getSignedInUserId(), post?.id);
    ...
    async function favouritePost() {
        await createFavourite(await getSignedInUserId(), post?.id);
        isFavourite = true;
    }

    async function unFavouritePost() {
        await deleteFavourite(await getSignedInUserId(), post?.id);
        isFavourite = false;
    }
</script>
...
<aside class="mb-3">
    <div class="flex justify-between items-center mx-4 mt-2.5 {post?.location ? 'mb-2' : ''}">
        <div class="flex gap-3 items-center">
            ...
            <a class="text-white" href={`/u/${post?.poster?.id}`}>{post?.poster?.username}</a>
        </div>
        <div>
...
<main class="w-full">
    ...
    <div>
        <div class="flex justify-between items-center m-4">
            ...
            <div>
                {#if !isFavourite}
                    <span on:click={async () => await favouritePost()} class="cursor-pointer">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                        </svg>
                    </span>
                {:else}
                    <span on:click={async () => await unFavouritePost()} class="cursor-pointer">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="#ed4956" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ed4956" class="w-8 h-8">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                        </svg>
                    </span>
