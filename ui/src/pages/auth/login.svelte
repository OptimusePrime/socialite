...
<!--<div>
    ...
    <main>
        ...
        <Button on:click={handleLogin} color="indigo" size="lg" className="mt-6">Login</Button>
    </main>
</div>-->
...
<div>
    ...
    <main class="flex flex-col gap-0">
        <Input className="mt-1 basis-full" placeholder="Email" size="lg" type="email" bind:value={email} bind:helperText={emailError} />
        <Input className="mt-1 basis-full" placeholder="Password" size="lg" type={showPassword ? "text" : "password"} bind:value={password} bind:helperText={passwordError}>
            <span class="hover:cursor-pointer" on:click={() => showPassword = !showPassword}>
                {#if showPassword}
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
        ...
        <Button on:click={handleLogin} className="mt-6 basis-full" color="indigo" size="lg">Login</Button>
    </main>
</div>
